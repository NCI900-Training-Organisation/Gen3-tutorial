...
This means that GEN3 graph model (data model) can be represented as
    - collection of `Gen3 Schema` files that are connected to each other through `links` (relationships).
    - Or single json file 
...
.. note::
    ...
    **Gen3 Schema**: A single yaml or json file that defines a single node in the data model.

    **Gen3 Data Dictionary**: A folder containing multiple yaml files for each node in the data model.

    **Gen3 Bundled Schema**: A json file containing a dictionary of jsonschemas for each node in the data model.
...
is used as the data dictionary for a given gen3 instance.
...
We will be using the `Gen3SchemaDev <https://github.com/AustralianBioCommons/gen3schemadev>`_ tool to create the data dictionary for our example data model.

First lets setup the `Gen3SchemaDev`.
...
.. note::
    ...
    In practuve the URL will be a link to an S3 bucket.


Next we will define the `nodes` in our data dictionary.

In our example we have the following nodes in the data model: `Patient`, `Blood`, `Urine`, `Plasma`, `DNA`, and `SeqRun`. 
...
Then we add node catogories to the data dictionary.
...
Then we add discription to eac node in the data dictionary.
...
For example we cann add an enum to the `blood_type` property in the `Blood` node to specify the 
allowed blood types.
